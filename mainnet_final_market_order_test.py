...
def main():
    ...
    COIN = "BTC"
    LEVERAGE = 5
    USD_AMOUNT = 1.0  # 风险控制：仅使用 1 美元进行本次市价单实盘测试
    IS_BUY = True  # True = 买入开多, False = 卖出开空

    print("--- Hyperliquid主网最终市价单测试脚本 ---")
        ...
        try:
            ...
            for status in order_result["response"]["data"]["statuses"]:
                if "filled" in status:
                    ...
                    print(f"2. 在 '持仓' (Positions) 面板中，您应该能看到一个新的 {COIN} {'多头' if IS_BUY else '空头'} 仓位。")
                    break # 找到第一个成交信息后即可退出循环
